...
class GraphGeneration(MongoConnection):
    ...
    def generate_id_batches(self, where, batch_size):
        """
        Generates ID boundaries to divide the data into batches based on the `_id` field.
        This method yields tuples of start and end _ids for each batch, allowing for efficient processing using $gt and $lte.
        ...
        last_id = None
        while True:
            # Create the query to find the next batch_size IDs
            query = {'$and': [where, {"_id": {"$gt": last_id}}]} if last_id is not None else dict(where)
            
            # Fetch only the _id field to minimize data transfer
            cursor = c.find(query, {"_id": 1}).sort([("_id", ASCENDING)]).limit(batch_size)
            ...
            # If we fetched less than batch_size, it's the last batch
    ...
    def worker_process(self, where, project, method, batch, batch_size, checkpoint_interval, process_id):
        """
        Worker function to process a batch of data from MongoDB.
        This function connects to the MongoDB, retrieves documents in the specified batch,
        processes them, and saves intermediate results to checkpoint files.

        :param where: MongoDB query filter.
        :param project: Fields to project in the MongoDB query.
        :param method: Processing method, either "full" or "day/week".
        :param batch: Tuple containing start and end _ids for the batch.
        ...
        # Create the query filter for the batch using _id boundaries
        d = {"_id": {"$gte": start_id, "$lte": end_id}}
        where_f = {'$and': [where, d]}
    ...
    def query_data_in_batches(self, where, project, method="full", user_batch_size=100000, checkpoint_interval=10000):
        """
        Queries data from MongoDB in batches and processes it using multiple workers in parallel.
        Each worker processes a batch of documents, saves user calculated results to checkpoint files, and merges the results at the end.

        :param where: MongoDB query filter.
        :param project: Fields to project in the MongoDB query.
        :param method: Processing method, either "full" or "day/week".
        :param user_batch_size: Number of documents to process in each batch.
        :param checkpoint_interval: Interval at which to save checkpoints.

        :return: None
        """
        batches = list(self.generate_id_batches(where, user_batch_size))

        Writer.create_dirs(self.output_file_path, self.id)

        max_workers = min(4, len(batches)) if len(batches) > 0 else 1
        futures = []

        self.logger.info(f"Processing {len(batches)} batches with {max_workers} workers.")

        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            for i, batch in enumerate(batches):
                self.logger.info(f"Submitting batch {i} to worker")
                future = executor.submit(
                    self.worker_process, where, project, method, batch, user_batch_size, checkpoint_interval, i)
        ...
        self.logger.info("All batches processed. Merging and aggregating results.")
