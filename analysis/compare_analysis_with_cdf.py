...
def plot_cdf(sizes_dict, resolution, output_dir):
    """Plots the CDF of community sizes for a given resolution."""
    ...
    for name, sizes in sizes_dict.items():
        ...
        # Sort sizes to prepare for CDF calculation
        sorted_sizes = np.sort(sizes)
        # The y-axis of the CDF is the cumulative probability
        cdf_y = np.arange(1, len(sorted_sizes) + 1) / len(sorted_sizes)
        
        plt.plot(sorted_sizes, cdf_y, marker='.', linestyle='none', alpha=0.6, label=f'{name} ({len(sizes):,} comms)')

    plt.xscale('log')
    plt.yscale('log')
    plt.xlabel("Community Size (Number of Users)")
    plt.ylabel("CDF (Fraction of communities <= size)")
    plt.title(f"Cumulative Distribution of Community Sizes at Resolution {resolution}")
    plt.legend()
    plt.grid(True, which="both", ls="--", c='0.7')
    
    # Add a vertical line for a potential threshold
    plt.axvline(x=10, color='r', linestyle='--', label='Threshold=10')
    
    # Save the plot
    plot_filename = os.path.join(output_dir, f'community_size_cdf_res_{resolution}.png')
...
def main(args):
    ...
    if True:
        ...
        # Load CSVs, ensuring empty fields are read as empty strings, not NaN
        print("Loading data...")
        ...
        for name, df in dataframes.items():
            ...
            if 'type' in df.columns:
                ...
                print(f"Warning: 'type' column not found in {name}. Assuming all nodes are users.")
        ...
        for res in resolutions:
            ...
            for name, df in dataframes.items():
                # Filter for only user nodes
                if 'type' in df.columns:
                    user_df = df[df['type'] == 'u']
                else:
                    user_df = df

                if res in user_df.columns and not user_df.empty:
                    # Exclude empty strings (unassigned nodes) before counting
                    counts = user_df[res].replace('', np.nan).dropna().value_counts()
                    sizes = counts.tolist()
                    ...
                    if sizes:
                        ...
                        frac_le_10 = np.sum(np.array(sizes) <= 10) / len(sizes)
                        print(f"  Graph '{name}': Found {len(sizes):,} communities. Median size: {median_size:.1f}. Fraction <= 10 users: {frac_le_10:.2%}")
                    else:
                        print(f"  Graph '{name}': No user communities found for this resolution.")

            # Plot the CDF for the current resolution
            plot_cdf(community_sizes, res, args.outdir)
